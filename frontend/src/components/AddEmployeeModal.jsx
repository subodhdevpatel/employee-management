...
const AddEmployeeModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  // Ensure form is cleared every time the modal is opened
  useEffect(() => {
    if (isOpen) {
      setForm(initialForm);
    }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[65] flex items-center justify-center p-4" onClick={onClose}>
      <div className="glass-strong rounded-3xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 sm:p-6 border-b border-white/10 flex items-center justify-between">
          <h2 className="text-xl sm:text-2xl font-display font-bold text-white">Add Employee</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <input name="name" placeholder="Name" value={form.name} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="email" type="email" placeholder="Email" value={form.email} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="age" type="number" placeholder="Age" value={form.age} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="department" placeholder="Department" value={form.department} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="position" placeholder="Position" value={form.position} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="phone" placeholder="Phone" value={form.phone} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
            <input name="salary" type="number" placeholder="Salary" value={form.salary} onChange={handleChange} className="px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-white" required />
